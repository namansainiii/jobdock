<x-layout>
    <x-slot name="title">Post a Job — JobDock</x-slot>

    {{-- Page Hero --}}
    <div class="bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900 py-10 px-4 text-center text-white mb-8">
        <div class="flex items-center justify-center gap-3 mb-2">
            <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                <i class="fas fa-briefcase text-white text-lg"></i>
            </div>
            <h1 class="text-3xl font-extrabold tracking-tight">Post a New Job</h1>
        </div>
        <p class="text-blue-200 text-sm">Fill in the details below to attract the best candidates.</p>
    </div>
    ...
    <div class="max-w-5xl mx-auto px-4 pb-16">
        ...
        <form method="POST" action="/jobs" enctype="multipart/form-data" class="space-y-6">
            @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="remote" id="remote"
                                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all bg-white">
                                <option value="0" {{ old('remote', '0') == '0' ? 'selected' : '' }}>On-site</option>
                                <option value="1" {{ old('remote') == '1' ? 'selected' : '' }}>Remote</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">Open Positions</label>
                            <input type="number" name="vacancies" id="vacancies" value="{{ old('vacancies', 1) }}"
                                min="1" max="999"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="salary" id="salary" value="{{ old('salary') }}"
                                    ...
                                    class="w-full border border-gray-200 rounded-xl pl-8 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all @error('salary') border-red-400 bg-red-50 @enderror">
                            </div>
                        </div>
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-1">Leave blank to show a single salary figure.</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6 space-y-5">
                    <div>
                        ...
                        <textarea name="description" id="description" rows="6"
                            ...
                            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none @error('description') border-red-400 bg-red-50 @enderror">{{ old('description') }}</textarea>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">Requirements</label>
                            <textarea name="requirements" id="requirements" rows="4"
                                placeholder="Bachelor's degree in CS, 3+ years React experience..."
                                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none">{{ old('requirements') }}</textarea>
                        </div>
                        <div>
                            ...
                            <textarea name="benefits" id="benefits" rows="4"
                                ...
                                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none">{{ old('benefits') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Address / Search Location</label>
                        <div class="relative">
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <input type="text" name="city" id="city" value="{{ old('city') }}" readonly
                                    class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm bg-gray-50 text-gray-600 @error('city') border-red-400 @enderror">
                            </div>
                            <div>
                                ...
                                <input type="text" name="state" id="state" value="{{ old('state') }}" readonly
                                    class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm bg-gray-50 text-gray-600 @error('state') border-red-400 @enderror">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">ZIP Code</label>
                                <input type="text" name="zipcode" id="zipcode" value="{{ old('zipcode') }}" readonly
                                    class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm bg-gray-50 text-gray-600">
                            </div>
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="company_description" id="company_description" rows="3"
                            ...
                            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none">{{ old('company_description') }}</textarea>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                        ...
                        <div class="border-2 border-dashed border-gray-200 rounded-xl p-4 flex items-center gap-4 hover:border-blue-300 transition-all">
                            ...
                            <div class="flex-1">
                                <input type="file" name="company_logo" id="company_logo" accept="image/*"
                                    class="block w-full text-sm text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-all">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                        <div>
                            ...
                            <select name="status" id="status"
                                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all bg-white">
                                <option value="active" selected>🟢 Active — visible to job seekers immediately</option>
                                <option value="draft">🟡 Draft — save privately, publish later</option>
                            </select>
                            <p class="text-xs text-gray-400 mt-1.5">You can change this anytime from your dashboard.</p>
                        </div>
...
function initMap() {
    ...
    map = new google.maps.Map(document.getElementById("map"), { center: defaultLocation, zoom: 5 });
